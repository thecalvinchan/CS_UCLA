...
{
    int pos = *aa-16;
    int pos_pay = hdr_payload(pos);
    int coalesce = 0;
    
    if (hdr_freeall(pos) == 0)
        return;
    
    //freeing the block
    hdr_freeall(pos) = 0;
    hdr_freeall(pos+pos_pay+16) = 0;
    
    //calculating the right neighbor's address
    int rneighbor = pos+32+pos_pay;
    
    //calculating the left neighbor's address
    int lpay = hdr_payload(pos-16);
    int lneighbor = pos-32-lpay;
    
    //if the neighbor to the right is free, coalesce with it
    if (rneighbor <= heapsize && hdr_freeall(rneighbor) == 0)
    {
        coalesce = 1;
        
        //adding the right neighbor's payload to the pos's payload
        int neighbor_pay = hdr_payload(rneighbor);
        hdr_payload(pos) += neighbor_pay+32;
        
        //zeroing out the payload of the right neighbor
        hdr_payload(rneighbor) = 0;
        hdr_payload(rneighbor-16) = 0;
        
        hdr_succesr(pos) = hdr_succesr(rneighbor);
        hdr_previus(pos) = hdr_previus(rneighbor);
        hdr_succesr(hdr_previus(pos)) = pos;
        hdr_previus(hdr_succesr(pos)) = pos;
        
        //updating the footer
        hdr_payload(rneighbor+neighbor_pay+16) = hdr_payload(pos);
    }
    
    //if the neighbor to the left is free, coalesce with it
    if (lneighbor >= 0 && hdr_freeall(lneighbor) == 0)
    {
        coalesce++;
        
        //adding the pos's payload to the left neighbor's payload
        hdr_payload(lneighbor) += hdr_payload(pos) + 32;
        
        //update the footer of the new big block
        hdr_payload(lneighbor+hdr_payload(lneighbor)+16) = hdr_payload(lneighbor);
        
        //zeroing out the payload of the hdr of pos and ftr of lneighbor
        hdr_payload(pos) = 0;
        hdr_payload(pos-16) = 0;
     
        //update anchor
        if (anchor.succesr == pos)
            anchor.succesr = lneighbor;
        
        //if the successor of the lneighbor has been coalesced, update successor
        if (coalesce == 2)
        {
            hdr_previus(hdr_succesr(pos)) = lneighbor;
            hdr_succesr(lneighbor) = hdr_succesr(pos);
        }
    }
    
    if (anchor.succesr > pos)
    {
        hdr_previus(pos) = hdr_previus(anchor.succesr);
        hdr_previus(anchor.succesr) = pos;
        hdr_succesr(pos) = anchor.succesr;
        anchor.succesr = pos;
    }
    if (coalesce == 0 && anchor.succesr < pos && hdr_succesr(anchor.succesr) > pos)
    {
        hdr_previus(pos) = anchor.succesr;
        hdr_succesr(pos) = hdr_succesr(anchor.succesr);
        hdr_previus(hdr_succesr(anchor.succesr)) = pos;
        hdr_succesr(anchor.succesr) = pos;
    }
}
...
void tfree( int *aa ) 
   {
      ...
      // Free block adjacent to the right
      ...
      if (existsFree == 0) {
         ...
         hdr_previus(anchor.succesr) = head;
         anchor.succesr = head;   
         hdr_previus(head) = -1;
      }
   }
